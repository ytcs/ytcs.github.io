...
- The total evidence density for a given context $$X$$ across all possible next tokens is $$\mathcal{E}_X = \sum_{k=1}^{N_V} E(y_k \mid X)$$. This sum can be rewritten as $$\mathcal{E}_X = \sum_{(X_j, y_{\text{target}}) \in \mathcal{D}} K(X, X_j)$$, so $$0 \le \mathcal{E}_X \le \mid\mathcal{D}\mid$$, the total size of the corpus.
...
$$
\bar{\mathcal{E}}_X = \mathbb{E}_{X}[\mathcal{E}_X] = \frac{\mid\mathcal{D}\mid \bar{S}}{N_V^{L_X-1}}
$$

Where:
- $$\mid\mathcal{D}\mid$$ is the size of the training corpus.

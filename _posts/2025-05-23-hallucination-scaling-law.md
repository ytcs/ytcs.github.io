...
An LLM with parameters $\theta$ models a conditional probability distribution $P(Y|X, \theta)$, where $X = (x_1, \ldots, x_{L_X-1})$ is the input context of length $L_X$, and $Y$ is the next token. The model outputs logits $\mathbf{l}(X, \theta) = (l_1, \ldots, l_{N_V})$, where $l_i$ is the logit for the $i$-th token in a vocabulary $V$ of size $N_V$.
...
$$
P(y_i|X, \theta) = \frac{\exp(l_i)}{\sum_{j=1}^{N_V} \exp(l_j)}
$$

When generating text with a temperature parameter $T > 0$, this probability is adjusted:

$$
P_T(y_i|X, \theta) = \frac{\exp(l_i/T)}{\sum_{j=1}^{N_V} \exp(l_j/T)}
$$
...
$$
P_{\text{hall}}(X, \theta, T) = 1 - P_T(y^*(X)|X, \theta)
$$
...
$$
E(y_k|X) = \sum_{(X_j, y_{\text{target}}) \in \mathcal{D} \text{ s.t. } y_{\text{target}} = y_k} K(X, X_j)
$$
...
- $0 \le E(y_k|X) \le N_{y_k}(\mathcal{D})$, where $N_{y_k}(\mathcal{D})$ is the total count of token $y_k$ in the training corpus $\mathcal{D}$. This is because the kernel $K(X,X_j)$ is between 0 and 1.
- The total evidence density for a given context $X$ across all possible next tokens is $\mathcal{E}_X = \sum_{k=1}^{N_V} E(y_k|X)$. This sum can be rewritten as $\mathcal{E}_X = \sum_{(X_j, y_{\text{target}}) \in \mathcal{D}} K(X, X_j)$, so $0 \le \mathcal{E}_X \le |\mathcal{D}|$, the total size of the corpus.
...
$$
l_i(X, \theta) \approx (\beta_S + \beta_C)E(y_i|X) - \beta_C \mathcal{E}_X + c
$$

This simplified model has a few key assumptions:
- Each token $y_i$ gains logit strength proportional to its own evidence $E(y_i|X)$ (scaled by $\beta_S + \beta_C$).
...
Now, let's consider what it takes for the hallucination probability $P_{\text{hall}}(X, \theta, T)$ for a *specific* context $X$ to be below a chosen threshold $\epsilon_0 \in (0,1)$. If $P_{\text{hall}}(X, \theta, T) \le \epsilon_0$, then the probability of generating the correct token $y^*(X)$ must be $P_T(y^*(X)|X, \theta) \ge 1 - \epsilon_0$.
...
$$
E(y^*(X)|X) - E(y_{\text{alt}}}|X) \ge \frac{T \log\left(\frac{1 - \epsilon_0}{\epsilon_0}\right)}{\beta_S + \beta_C}
$$
...
If $E(y^*(X)|X) - E(y_{\text{alt}}}|X) < M'_{\text{evidence}}(\epsilon_0, T)$, then $P_{\text{hall}}(X, \theta, T) > \epsilon_0$.
...
$$
\bar{\mathcal{E}}_X = \mathbb{E}_{X}[\mathcal{E}_X] = \frac{|\mathcal{D}| \bar{S}}{N_V^{L_X-1}}
$$

Where:
- $|\mathcal{D}|$ is the size of the training corpus.
...
The ratio then becomes $\frac{\bar{\mathcal{E}}_X}{M'_{\text{evidence}}(\epsilon_0, T)} = \frac{C_{model} |\mathcal{D}|}{N_V^{L_X-1} T L(\epsilon_0)}$.
So, the lower bound on $f(\epsilon_0)$ is:

$$
f(\epsilon_0) \ge 1 - \frac{C_{model} |\mathcal{D}|}{N_V^{L_X-1} T L(\epsilon_0)}
$$
...
$$
\bar{P}_{\text{hall}} > \epsilon_0 \left(1 - \frac{C_{model} |\mathcal{D}|}{N_V^{L_X-1} T L(\epsilon_0)}\right)
$$

This inequality gives us a lower bound on the average hallucination probability. Let's break down the term $k_0(\epsilon_0) = \frac{C_{model} |\mathcal{D}|}{N_V^{L_X-1} T L(\epsilon_0)}$:
- $C_{model} |\mathcal{D}|$ represents the 'total effective evidence' in the corpus, scaled by model and kernel parameters.
...
Let $k = \frac{C_{model} |\mathcal{D}|}{N_V^{L_X-1} T}$ be the key scaling parameter, which encapsulates data density ($|\mathcal{D}|/N_V^{L_X-1}$), model characteristics ($C_{model}$), and inverse temperature ($1/T$). The bound is $g(\epsilon_0) = \epsilon_0 (1 - \frac{k}{L(\epsilon_0)})$.
...
*Figure 1: The optimized lower bound on average hallucination probability, $\bar{P}_{\text{hall}}$, as a function of the data density, model, and temperature parameter $k = \frac{C_{model} |\mathcal{D}|}{N_V^{L_X-1} T}$. The bound transitions from $\approx 0.5$ for small $k$ (sparse data/high temperature) towards $0$ for large $k$ (dense data/low temperature).*

## Conclusion

This derivation provides a theoretical lower bound on the average hallucination probability in LLMs, starting from a model of how corpus evidence translates to logits. It formally demonstrates how this hallucination floor scales with fundamental parameters, encapsulated in the parameter $k = \frac{C_{model} |\mathcal{D}|}{N_V^{L_X-1} T}$:

-   **Corpus Size ($|\mathcal{D}|$):** Larger datasets tend to decrease the bound (increase $k$).

...
$$
P_{\text{h}}(X, \theta, T) = 1 - P_T(y^*(X) | X, \theta)
$$
...
$$
l_i(X, \theta) \approx \beta_S E(y_i | X) - \beta_C \mathcal{E}_X + c
$$

This simplified model has a few key assumptions:
- Each token $$y_i$$ gains logit strength proportional to its own evidence $$E(y_i | X)$$ (scaled by $$\beta_S$$).
...
$$
E(y^*(X) | X) - E(y_{\text{alt}} | X) \ge \frac{T \log\left(\frac{1 - \epsilon_0}{\epsilon_0}\right)}{\beta_S}
$$
...
If $$E(y^*(X) | X) - E(y_{\text{alt}} | X) < M'(\epsilon_0, T)$$, then $$P_{\text{h}}(X, \theta, T) > \epsilon_0$$.
...
The ratio then becomes $$\frac{\bar{\mathcal{E}}_X}{M'(\epsilon_0, T)} = \frac{C |\mathcal{D}|}{N_V^{L_X-1} T L(\epsilon_0)}$$.
So, the lower bound on $$f(\epsilon_0)$$ is:

$$
f(\epsilon_0) \ge 1 - \frac{C |\mathcal{D}|}{N_V^{L_X-1} T L(\epsilon_0)}
$$
...
$$
\bar{P}_{\text{h}} > \epsilon_0 \left(1 - \frac{C |\mathcal{D}|}{N_V^{L_X-1} T L(\epsilon_0)}\right)
$$

This inequality gives us a lower bound on the average hallucination probability. Let's break down the term $$k_0(\epsilon_0) = \frac{C |\mathcal{D}|}{N_V^{L_X-1} T L(\epsilon_0)}$$:
- $$C |\mathcal{D}|$$ represents the 'total effective evidence' in the corpus, scaled by model and kernel parameters.
...
Let $$k = \frac{C |\mathcal{D}|}{N_V^{L_X-1} T}$$ be the key scaling parameter, which encapsulates data density ($$|\mathcal{D}|/N_V^{L_X-1}$$), model characteristics ($$C$$), and inverse temperature ($$1/T$$). The bound is $$g(\epsilon_0) = \epsilon_0 (1 - \frac{k}{L(\epsilon_0)})$$.
...
This derivation provides a theoretical lower bound on the average hallucination probability in LLMs, starting from a model of how corpus evidence translates to logits. It formally demonstrates how this hallucination floor scales with fundamental parameters, encapsulated in the parameter $$k = \frac{C |\mathcal{D}|}{N_V^{L_X-1} T}$$:

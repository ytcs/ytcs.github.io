...
Our discussion will draw heavily from the comprehensive review by Wolpert [arXiv:1905.05669v2](https://arxiv.org/abs/1905.05669), aiming to make its key insights accessible and highlight the physical intuition behind the mathematical formalism.
...
A CTMC describes a system that can occupy a discrete set of states $$X$$. The system stochastically jumps between these states, and the probability $$p_t(x)$$ of finding the system in state $$x \in X$$ at time $$t$$ evolves according to a master equation:

$$ 
\frac{dp_t(x)}{dt} = \sum_{x' \neq x} [K_t(x\vertx')p_t(x') - K_t(x'\vertx)p_t(x)] 
$$

Here, $$K_t(x\vertx')$$ is the time-dependent transition rate from state $$x'$$ to state $$x$$. We can also write this in matrix form as $$ \frac{d \vec{p}_t}{dt} = \mathbf{K}_t \vec{p}_t $$, where $$\mathbf{K}_t$$ is the rate matrix with off-diagonal elements $$K_t(x\vertx')$$ and diagonal elements $$K_t(x\vertx) = - \sum_{x' \neq x} K_t(x'\vertx)$$.
...
$$ 
    \mathcal{Q}_t = \frac{1}{2} \sum_{x, x'} [K_t(x\vertx')p_t(x') - K_t(x'\vertx)p_t(x)] \ln \frac{K_t(x\vertx')}{K_t(x'\vertx)} 
    $$

    This expression is particularly relevant under the condition of *local detailed balance*, where the ratio of forward and backward rates is related to the energy change: $$K_t(x\vertx') / K_t(x'\vertx) = e^{-\Delta E_{x,x'}/T}$$. In such cases, EF represents the heat dissipated to the environment, divided by temperature.
...
$$ 
    \mathcal{E}_t = \frac{1}{2} \sum_{x, x'} [K_t(x\vertx')p_t(x') - K_t(x'\vertx)p_t(x)] \ln \frac{K_t(x\vertx')p_t(x')}{K_t(x'\vertx)p_t(x)} 
    $$

    EP is zero if and only if the system is in its stationary state $$\pi$$ and the detailed balance condition $$K_t(x\vertx')\pi(x') = K_t(x'\vertx)\pi(x)$$ holds for all $$x, x'$$ (i.e., the system is in thermodynamic equilibrium).
...
$$ S(X\vertY) = S(X,Y) - S(Y) = - \sum_{x,y} p(x,y) \ln p(x\verty) $$
...
$$ D(p \vert\vert q) = \sum_x p(x) \ln \frac{p(x)}{q(x)} $$
    ...
    It is always non-negative ($$D(p\vert\vertq) \ge 0$$) and equals zero if and only if $$p=q$$. Note that it is not symmetric, i.e., $$D(p\vert\vertq) \neq D(q\vert\vertp)$$ in general.
...
$$ I(X;Y) = S(X) - S(X\vertY) = S(Y) - S(Y\vertX) = S(X) + S(Y) - S(X,Y) $$
...
*   **Cross-Entropy**: While not as central to the core arguments of Wolpert (2019) that we will focus on, cross-entropy $$H(p,q) = - \sum_x p(x) \ln q(x) = S(p) + D(p\vert\vertq)$$ is another related measure often encountered in information theory and machine learning.
...
We define the **(unconstrained) Landauer cost** of the logical map $$\map$$ acting on an initial distribution $$p_{initial}$$ as the reduction in Shannon entropy:

$$ \mathcal{L}(p_{initial}, \map) = S(p_{initial}) - S(p_{final}) $$

Thus, Landauer's principle states that the minimum heat dissipated by the system is:

$$ \mathcal{Q}_{dissipated} \ge T \mathcal{L}(p_{initial}, \map) $$

If we set $$k_B T = 1$$ as often done in this context for simplicity (making $$T=1$$ if $$k_B=1$$), the bound becomes:

$$ \mathcal{Q}_{dissipated} \ge \mathcal{L}(p_{initial}, \map) $$

This means that any logical operation that reduces the Shannon entropy of the system (i.e., $$\mathcal{L} > 0$$, such as erasing a bit) must dissipate at least this amount of heat. The minimum dissipation is achieved when the process is thermodynamically reversible (i.e., total system entropy production $$\mathcal{E}_{sys}=0$$).

Let's connect this to the notation used earlier in this post. We had $$\Delta S = \mathcal{Q} + \mathcal{E}$$, where $$\mathcal{Q}$$ was the total Entropy Flow (EF) *into* the system, and $$\mathcal{E}$$ was the total system Entropy Production (EP, which is non-negative, $$\mathcal{E} \ge 0$$). 
The entropy flow into the system, $$\mathcal{Q}$$, is related to the heat dissipated by the system to the environment, $$\mathcal{Q}_{dissipated}$$, by $$\mathcal{Q} = - \mathcal{Q}_{dissipated}/T$$. 
...
Since the Landauer cost is defined as $$\mathcal{L}(p_{initial}, \map) = S(p_{initial}) - S(p_{final}) = - \Delta S$$, we can substitute this to get:

$$ \frac{\mathcal{Q}_{dissipated}}{T} = \mathcal{E} + \mathcal{L}(p_{initial}, \map) $$

This fundamental equation shows that the total heat dissipated (per unit temperature) is the sum of the Landauer cost (the unavoidable cost due to information change) and the system's entropy production $$\mathcal{E}$$ (the cost due to the process's irreversibility). Since $$\mathcal{E} \ge 0$$, this directly confirms Landauer's principle: $$\mathcal{Q}_{dissipated}/T \ge \mathcal{L}(p_{initial}, \map)$$. The minimum dissipation occurs when $$\mathcal{E} = 0$$, i.e., the process is thermodynamically reversible. The subsequent discussion of mismatch cost and residual EP details the components that constitute this $$\mathcal{E}$$.

### Mismatch Cost and Intrinsic Dissipation

The total entropy production $$\mathcal{E}_{proc}(p)$$ for a given physical realization of the map $$\map$$ (denoted by `proc`) acting on an initial distribution $$p$$ can be decomposed (Wolpert, Sec 5.2):

$$ \mathcal{E}_{proc}(p) = \mathcal{M}_{proc}(p) + \mathcal{E}_{res;proc}(p) $$

Here:

1.  **Mismatch Cost ($$\mathcal{M}_{proc}(p)$$)**: Any physical process is typically designed or optimized assuming a certain input probability distribution, let's call it $$q^{proc}$$. If the actual input distribution $$p$$ differs from $$q^{proc}$$, there's an additional thermodynamic cost. This is the mismatch cost, given by:
    
$$ \mathcal{M}_{proc}(p) = D(p \vert\vert q^{proc}) - D(\map p \vert\vert \map q^{proc}) $$

    where $$D(\cdot \vert\vert \cdot)$$ is the KL divergence. The mismatch cost represents the inefficiency incurred because the device is not operating on its 'expected' input. It arises from the mismatch between the actual statistics of the input and the statistics the physical dynamics were implicitly or explicitly designed to handle optimally. Note that $$\map q^{proc}$$ is the output distribution that would result if the input were indeed $$q^{proc}$$.

2.  **Residual Entropy Production ($$\mathcal{E}_{res;proc}(p)$$)**: This is the entropy production that occurs even if the input distribution perfectly matches the designed-for prior (i.e., if $$p = q^{proc}$$, then $$\mathcal{M}_{proc}(p) = 0$$). It represents the intrinsic irreversibility of the physical process chosen to implement the map $$\map$$. For example, performing a transformation too quickly or using a suboptimal control protocol will generally lead to non-zero residual EP. Wolpert shows that $$\mathcal{E}_{res;proc}(p)$$ is the average, over certain partitions of the state space (islands), of the EP intrinsic to the process when fed its designed-for prior $$q^{proc}$$.

Combining these, the total heat dissipated by the system is:

$$ \mathcal{Q}_{dissipated} = \mathcal{L}(p, \map) + \mathcal{M}_{proc}(p) + \mathcal{E}_{res;proc}(p) $$

This equation provides a more complete picture of the thermodynamic costs:
*   The **Landauer cost** $$\mathcal{L}(p, \map)$$ is the fundamental, unavoidable dissipation tied directly to the change in information content (Shannon entropy) by the logical map itself. It's a lower bound if the process is perfectly matched to the input and executed reversibly.
...
For an ideal scenario, often referred to as an "All-at-Once" (AO) device (Wolpert, Sec 8), it's assumed that $$\mathcal{E}_{res;proc}(p) = 0$$. In this case, $$\mathcal{Q}_{dissipated} = \mathcal{L}(p, \map) + \mathcal{M}_{proc}(p)$$. If, additionally, the input distribution $$p$$ matches the device's prior $$q^{proc}$$, then $$\mathcal{M}_{proc}(p) = 0$$, and we recover the simple Landauer bound: $$\mathcal{Q}_{dissipated} = \mathcal{L}(p, \map)$$.
...
*   **Logical Reversibility**: A logical map $$\map: X \rightarrow Y$$ is reversible if it is bijective, meaning that for every output $$y \in Y$$, there is a unique input $$x \in X$$ such that $$\map(x) = y$$. In simpler terms, you can uniquely determine the input if you know the output. If a map is logically reversible, then the input and output spaces must have the same size, and the map merely permutes the states. Consequently, for any distribution $$p$$ on $$X$$, the entropy of the output distribution $$S(\map p)$$ is equal to the entropy of the input distribution $$S(p)$$. This means the **Landauer cost for any logically reversible map is always zero**:
    
$$
    \mathcal{L}(p, \map_{rev}) = S(p) - S(\map_{rev} p) = S(p) - S(p) = 0
    $$
...
1.  A **logically irreversible** map to be implemented by a **thermodynamically reversible** physical process. For example, the erasure of a bit (e.g., mapping $$\{0,1\}$$ to $$\{0\}$$) is logically irreversible. If the initial state is uniformly random ($$p(0)=p(1)=1/2$$), and the final state is fixed ($$S(\map p)=0$$), the Landauer cost is $$\mathcal{L} = \ln 2$$. This erasure can, in principle, be performed by a physical process that is thermodynamically reversible ($$\mathcal{E}_{proc}=0$$). In this case, the heat dissipated would be exactly $$\mathcal{Q}_{dissipated} = \mathcal{L} = \ln 2$$. This is the scenario Landauer originally considered.
...
Under Standard Accounting, the Landauer cost for the computational device performing a single map $$\map$$ from its input $$X^{IN}$$ (with initial distribution $$p_0(X^{IN})$$) to its output $$X^{OUT}$$ (with final distribution $$p_1(X^{OUT}) = \map p_0(X^{IN})$$) is thus:
...
Consider a circuit $$\mathcal{C}$$ that implements an overall logical map $$\map_{\mathcal{C}}$$ from the circuit's primary inputs $$X^{IN}$$ to its primary outputs $$X^{OUT}$$. An AO device could, in principle, implement the same map $$\map_{\mathcal{C}}$$ with a minimal dissipated heat of $$\mathcal{L}(p(X^{IN}), \map_{\mathcal{C}}) + \mathcal{M}_{AO}(p(X^{IN}))$$ (assuming $$\mathcal{E}_{res,AO}=0$$).
...
More formally, if a system composed of subsystems A and B evolves such that A and B process information independently ($$\map^{A,B} = \map^A \map^B$$), the total Landauer cost of this joint operation is $$\mathcal{L}_A + \mathcal{L}_B$$. The Landauer cost for an AO device performing the same global transformation on $$(A,B)$$ is $$\mathcal{L}_{A,B}$$. The Landauer Loss is the difference (Sec 9, Eq. 98):
...
$$ \text{Landauer Loss} = I_{p_0}(A;B) - I_{\map p_0}(A;B) $$

where $$p_0$$ is the initial joint distribution and $$\map p_0$$ is the final one. If the independent evolution of A and B destroys correlations between them (i.e., $$I_{\map p_0}(A;B) < I_{p_0}(A;B)$$), there is a positive Landauer Loss, meaning the constrained system dissipates more heat than an equivalent AO device, even if each subsystem process $$\map^A$$ and $$\map^B$$ is thermodynamically reversible.

For a straight-line circuit, the total dissipated heat will be the sum of Landauer costs of individual gates $$\sum_g \mathcal{L}_g$$, plus their mismatch costs $$\sum_g \mathcal{M}_g$$ and residual EPs $$\sum_g \mathcal{E}_{res,g}$$. The **Circuit Landauer Loss** is the amount by which $$\sum_g \mathcal{L}_g$$ (the sum of *local* Landauer costs) exceeds the *global* Landauer cost $$\mathcal{L}(p(X^{IN}), \map_{\mathcal{C}})$$:

$$ \Delta \mathcal{L}_{\mathcal{C}}(p) = \left( \sum_g \mathcal{L}_g(p_{pa(g)}) \right) - \mathcal{L}(p(X^{IN}), \map_{\mathcal{C}}) $$

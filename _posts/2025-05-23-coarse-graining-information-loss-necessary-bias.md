...
The microstate space \\(\Omega\\) (with alphabet \\(\mathcal{X}\\)) and the macrostate space \\(\Gamma\\) (with alphabet \\(\mathcal{K}\\)) are finite sets. Let \\(N = |\mathcal{X}|\\) and \\(M = |\mathcal{K}|\\).
...
Let \\(p(x)\\) be the uniform distribution over \\(\mathcal{X}\\) (\\(|\mathcal{X}|=N\\)), so \\(H(X) = \log_2 N\\). Let \\(\mathcal{F}_{N,M}\\) be the set of all surjective deterministic maps \\(f: \mathcal{X} \to \mathcal{K}\\) (\\(|\mathcal{K}|=M\\), \\(M \le N\\)). If a map \\(f\\) is chosen uniformly at random from \\(\mathcal{F}_{N,M}\\), let \\(K=f(X)\\) and \\(p_f(k)\\) be the resulting macrostate distribution. Let \\(W_k = |f^{-1}(k)|\\). Then:
...
3.  The expected information loss is bounded below: \\(\mathbb{E}_f[H(X|K)] \ge \log_2(N/M)\\).
4.  In the asymptotic limit \\(N \to \infty\\) with \\(M\\) fixed, the retained information concentrates: \\(H(K) \xrightarrow{P} \log_2 M\\).
5.  In the asymptotic limit \\(N \to \infty\\) with \\(M\\) fixed, the information loss concentrates: \\(H(X|K) \xrightarrow{P} \log_2(N/M)\\).
...
From the chain rule for entropy, we have \\(H(X|K) = H(X) - H(K)\\).
Taking the expectation over \\(f\\):
\\[ \mathbb{E}_f[H(X|K)] = \mathbb{E}_f[H(X) - H(K)] = H(X) - \mathbb{E}_f[H(K)] \\]
...
\\[ \mathbb{E}_f[H(X|K)] \ge \log_2 N - \log_2 M = \log_2(N/M) \\]
This proves part (3).
...
Using the chain rule again: \\(H(X|K) = H(X) - H(K) = \log_2 N - H(K)\\).
We want to show that \\(H(X|K) - \log_2(N/M) \xrightarrow{P} 0\\) as \\(N \to \infty\\) with \\(M\\) fixed.

Consider the difference:
\\[ H(X|K) - \log_2(N/M) = (\log_2 N - H(K)) - (\log_2 N - \log_2 M) = \log_2 M - H(K) \\]
...
This proves the convergence: \\(H(X|K) \xrightarrow{P} \log_2(N/M)\\). This proves part (5).

**Implications of Proposition 2.1:**
For large microstate spaces (\\(N\\)) and a fixed, much smaller number of macrostates (\\(M\\)), a typical (randomly chosen) coarse-graining map inevitably discards an amount of information \\(H(X|K)\\) that approaches \\(\log_2(N/M)\\). This lost information grows logarithmically with \\(N\\). Since the total information is \\(H(X) = \log_2 N\\), the fraction of information lost, \\(H(X|K)/H(X)\\), approaches \\((\log_2 N - \log_2 M) / (\log_2 N) = 1 - (\log_2 M) / (\log_2 N)\\). As \\(N \to \infty\\) for fixed \\(M\\), this fraction approaches 1.
*Example (Coin Flips): With \\(N=2^{100}\\) microstates (sequences) and \\(M=101\\) macrostates (number of heads), the retained information \\(H(K) \approx \log_2 101 \approx 6.66\\) bits. The lost information \\(H(X|K) \approx \log_2(2^{100}/101) \approx 100 - 6.66 = 93.34\\) bits. Over \\(93\%\\) of the original information is lost.*

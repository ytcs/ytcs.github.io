...
3.  **Average Conditional Entropy (Information Loss):** \\(H(X \\mid K) := \\sum_{k \\in \\mathcal{K}} p(k) H(X \\mid K=k) = \\mathbb{E}_K [H(X \\mid K=k)]\\). This quantity represents the average information lost about the microstate \\(X\\) when only the macrostate \\(K\\) is known.
    *If \\(p(x)=1/N\\), then \\(H(X \\mid K) = \\sum_{k \\in \\mathcal{K}} (W_k/N) \\log_2 W_k = \\mathbb{E}_K[\\log_2 W_K]\\).*

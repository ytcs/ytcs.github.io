...
**Methodology:**
Let $$\mathbf{a}^{(\ell)}(\mathbf{x})$$ be the activation vector at layer $$\ell$$ for input $$\mathbf{x}$$. Let $$\mathbf{d}_f$$ be the direction corresponding to feature $$f$$ (e.g., a column of an SAE decoder $$\mathbf{W}_d$$). The activation of feature $$f$$ might be directly given by an SAE as $$a_f(\mathbf{x})$$, or for a generic direction in $$\mathbf{a}^{(\ell)}(\mathbf{x})$$, it is $$\mathbf{d}_f^T \mathbf{a}^{(\ell)}(\mathbf{x})$$.

When processing $$\mathbf{x}_{\text{base}}$$, we run the model up to layer $$\ell$$. We then compute the activation vector $$\mathbf{a}^{(\ell)}(\mathbf{x}_{\text{base}})$$. We want to modify this vector such that the component corresponding to feature $$f$$ matches its activation from $$\mathbf{x}_{\text{source}}$$. 

If we are intervening on a feature $$f$$ represented by a specific direction $$\mathbf{d}_f$$ (normalized to unit length) within the activation vector $$\mathbf{a}^{(\ell)}$$, the original component of $$\mathbf{a}^{(\ell)}(\mathbf{x}_{\text{base}})$$ along $$\mathbf{d}_f$$ is $$( \mathbf{d}_f^T \mathbf{a}^{(\ell)}(\mathbf{x}_{\text{base}}) ) \mathbf{d}_f$$. The new component should be $$( \mathbf{d}_f^T \mathbf{a}^{(\ell)}(\mathbf{x}_{\text{source}}) ) \mathbf{d}_f$$. Thus, the modified activation vector at layer $$\ell$$, $$\mathbf{a}^{(\ell)}_{\text{patched}}$$, can be constructed by:
...
Alternatively, and more commonly when working with features from a sparse autoencoder (SAE), if the original activation $$\mathbf{x}^{(\ell)}$$ is replaced by its SAE reconstruction $$\hat{\mathbf{x}}^{(\ell)} = \mathbf{W}_d \mathbf{f}$$, where $$\mathbf{f}$$ is the vector of SAE feature activations, the intervention is simpler. We run the model with $$\mathbf{x}_{\text{base}}$$ to get SAE features $$\mathbf{f}(\mathbf{x}_{\text{base}})$$, and similarly $$\mathbf{f}(\mathbf{x}_{\text{source}})$$. For the target SAE feature $$f_k$$ (the $$k$$-th element in $$\mathbf{f}$$), we create a patched feature vector $$\mathbf{f}_{\text{patched}}$$ where $$(f_{patched})_k = f_k(\mathbf{x}_{\text{source}})$$ and all other elements $$(f_{patched})_{j \neq k} = f_j(\mathbf{x}_{\text{base}})$$. The patched activation for the main model is then $$\hat{\mathbf{x}}^{(\ell)}_{\text{patched}} = \mathbf{W}_d \mathbf{f}_{\text{patched}}$$. The model then continues its forward pass from layer $$\ell$$ using this patched activation. The downstream consequences (e.g., changes in output probabilities, internal states of later layers) are then measured.
